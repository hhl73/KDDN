...
if __name__ == "__main__":
    ...
    class trainer_2:
        def __init__(self, train_gen, step, numD=4):
            ...
            self.teacher = torch.load("./models/Teacher/Teacher.pkl")
            self.numd = numD
            self.step = step
            self.trainloader = train_gen
            self.modelG = Student2().to(device)
            self.optimizer_G = torch.optim.Adam(filter(lambda p: p.requires_grad, self.modelG.parameters()), lr=1e-6,
                                                betas=(0.9, 0.999))
        ...
        def opt_G1(self, Jx, fake,interfakes):
            self.optimizer_G.zero_grad()

            g_loss_MSE0 = self.criterion(fake, Jx.detach())
            PT, feaT = self.teacher(Jx.detach())
            lossRM = 0
            for i in range(len(interfakes)):
                # feaT[i] = stop_gradient(feaT[i])
                lossRM += self.criterion(interfakes[i], feaT[i])
            lossVGG = self.VGGLoss(fake, Jx.detach())
            loss = g_loss_MSE0 + lossRM + lossVGG
            loss.backward()

            self.optimizer_G.step()

            return g_loss_MSE0, lossRM, lossVGG
        ...
        def train(self, epoch, train_gen):
            ...
            for (Ix, Jx) in self.trainloader:
                ...
                fake, interfakes = self.modelG(Ix)

                loss1, loss2, loss3 = self.opt_G1(Jx, fake,interfakes)
                ...
                if iteration % 100 == 0:
                    ...
                    print("MSE:{:4f},MSSIM:{:4f},VGG:{:4f},VGG:{:4f}".format(loss1, loss1, loss2, loss3))
    ...
    for i in range(1, 2):
        ...
        for epoch in range(1, 30):
            ...
            torch.save(trainModel.modelG, "./models/Student/Student.pkl")